#!/usr/bin/env python3
"""
Walk-Forward Validation: Does the Sell edge persist across time periods?

Instead of a single train/holdout split, retrains on rolling windows and
tests on successive out-of-sample periods. If the edge is stable across
all windows, it's real. If it only works in one window, we're curve-fitting.

Uses the same conservative params, Sell-only filter, and regime filter.

Output saved to logs/walk_forward.log
"""
...
LOG_FILE = Path(__file__).parent / "logs" / "walk_forward.log"
...
def simulate_trades(df, sell_mask, tp_pips, sl_pips, max_horizon, commission,
                    cb_enabled=False, cb_max_losses=5, cb_max_dd_pips=100,
                    cb_cooldown=48, cb_reset_on_win=True):
    """Run TP/SL simulation on sell signals with optional circuit breaker."""
    pip_value = 0.0001
    closes = df["close"].values
    highs = df["high"].values
    lows = df["low"].values
...
def main():
    ...
    print("=" * 90)
    print("WALK-FORWARD VALIDATION — Is the Sell edge stable across time?")
    print("=" * 90)
    ...
    # Try to get datetime info for window labels
    ...
    # Run both without and with circuit breaker
    for run_label, cb_on in [("WITHOUT circuit breaker", False), ("WITH circuit breaker", True)]:
        print(f"\n{'=' * 90}")
        print(f"  {run_label}")
        print(f"{'=' * 90}\n")

        print(f"{'Window':>8} | {'Train':>8} | {'Test':>6} | {'Period':>25} | "
              f"{'Sells':>5} | {'Regime':>6} | {'Trades':>6} | {'WinR':>5} | "
        ...
        for w in range(n_windows):
            ...
            sell_mask_regime = sell_mask & regime_mask
            n_regime = int(sell_mask_regime.sum())

            if n_regime > 0:
                true_pos = (sell_mask_regime & (y_test == 0)).sum()
                precision = true_pos / n_regime
            else:
                precision = 0.0

            trades, triggers, skipped = simulate_trades(
                test_df, sell_mask_regime, tp_pips, sl_pips, max_horizon, commission,
                cb_enabled=cb_on, cb_max_losses=cb_max_losses,
            ...
            cb_info = f" [CB:{triggers}t,{skipped}s]" if cb_on and triggers > 0 else ""
            print(f"  W{w+1:>4}   | {train_end:7d}  | {test_end - test_start:5d} | {period:>25} | "
                  f"{n_sells:4d}  | {n_regime:5d} | {len(trades):5d}  | {win_rate:4.1%} | "
                  f"{precision:4.1%} | {total_pips:+6.0f} | {sharpe:+5.2f}{cb_info}")
            ...
            window_summaries.append({
                ...
                'period': period,
                'sell_signals': n_sells,
                'regime_signals': n_regime,
                'trades': len(trades),
                'win_rate': win_rate,
                'precision': precision,
                'total_pips': total_pips,
                'sharpe': sharpe,
                'cb_triggers': triggers if cb_on else 0,
            })
        ...
        if all_trade_results:
            ...
            if cb_on:
                total_cb = sum(s['cb_triggers'] for s in window_summaries)
